...
echo -n | openssl s_client -showcerts -connect ${JUMPHOST}:443 2>/dev/null | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > /tmp/ocp-registry.crt
sudo cp -f /tmp/ocp-registry.crt /etc/pki/ca-trust/source/anchors
sudo update-ca-trust extract 
sudo openssl verify /tmp/ocp-registry.crt 

echo Update "${LOCAL_SECRET_JSON}"
cp ${LOCAL_SECRET_JSON}.template ${LOCAL_SECRET_JSON}
...
EOF
cat ./Dockerfile
podman build -t ${JUMPHOST}/ubi8:latest -f Dockerfile
podman push  ${JUMPHOST}/ubi8:latest
...
sed -i "s/JUMPHOST/${JUMPHOST}/g" 0004-cluster-wide-entitled-pod-disconn.yaml
